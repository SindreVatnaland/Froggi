<script lang="ts">
	import { bingoSession, bingoLobby, electronEmitter, currentPlayer, urls, remoteAccess, obsConnection, ngrokStatus, bingoRevertMessage } from '$lib/utils/store.svelte';
	...
	const winConditions: { value: BingoWinCondition; label: string; tip: string }[] = [
		...
		{ value: 'full',    label: 'Full Board', tip: 'Complete every box on the board to win' },
		{ value: 'lockout', label: 'Lockout',    tip: 'Each box can only be claimed by one player. First to the majority wins. Neither player can complete a box the opponent has already taken.' },
	];
	...
	$: if (isActive && session?.settings?.timer?.enabled) {
	...
	$: localWinBoxes = getWinBoxesFiltered(board.boxes, size,
		b => b.completedBy === 'local' || b.completedBy === 'both');
	$: oppWinBoxes = getWinBoxesFiltered(board.boxes, size,
		b => b.completedBy === 'opponent' || b.completedBy === 'both');
...
<main class="background-primary-color text-secondary-color flex justify-center">
	<div class="w-full max-w-2xl flex flex-col gap-5">
		...
		{#if localOverlayUrl}
			<div class="settings-row border-secondary items-center justify-between gap-3">
				...
				<div class="flex gap-2 shrink-0">
					...
					<button
						class="btn text-sm h-9 px-4 border-secondary rounded disabled:opacity-40"
						disabled={!isObsConnected}
						on:click={() => $electronEmitter.emit('ObsCreateBrowserSource', localOverlayUrl, 'Bingo', { width: 500, height: 500 })}
					>
						{isObsConnected ? 'Add to OBS' : 'OBS not connected'}
					</button>
		...
		<!-- Timer -->
		{#if isActive && timerSecondsLeft !== null}
			<div class="timer-bar border-secondary" class:timer-bar--urgent={timerSecondsLeft <= 300}>
		...
		<!-- Win banner -->
		{#if hasWon}
			<div class="win-banner border-secondary">Bingo!</div>
		{/if}
			...
			<div class="info-modal-body">
				...
				<div class="info-section">
					...
					<ul class="info-list">
						...
						<li><strong>Full Board</strong> — Complete every box on the board to win.</li>
						<li><strong>Lockout</strong> — Each box can only be claimed by one player. First to the majority of boxes wins. You cannot complete a box your opponent has already taken.</li>
					</ul>
				</div>
